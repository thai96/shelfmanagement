...
public class SensitiveDataMaskingConverter extends MessageConverter {
    ...
    public String convert(ILoggingEvent event) {
        ...
        message = CONFIDENTAL_PATTERNS[0].matcher(message).replaceAll("$1=***");
        message = CONFIDENTAL_PATTERNS[1].matcher(message).replaceAll("$1***");
        message = CONFIDENTAL_PATTERNS[2].matcher(message).replaceAll("$1***$3");
        message = CONFIDENTAL_PATTERNS[3].matcher(message).replaceAll("$1***");
        return message;
    }
}